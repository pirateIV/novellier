        ...
        <TabList className="p-1.5 rounded-md bg-white dark:bg-zinc-950">
          <div className="grid grid-cols-4 justify-between gap-0.5 bg-neutral-100 dark:bg-[#0f0f0f] p-1 *:py-2.5 rounded-md">
            {tabs.map((tab) => (
              <Tab
                key={tab}
                className="text-center text-sm font-medium rounded text-gray-600 dark:text-gray-400 data-[selected]:text-gray-900 data-[selected]:bg-white dark:data-[selected]:bg-zinc-800 data-[selected]:shadow-sm dark:data-[selected]:border-b-zinc-700/60 outline-none"
              >
                {tab}
              </Tab>
            ))}
