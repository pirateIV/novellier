...
import { Star, ThumbsUp, Share2, Flag, Edit, Trash2, ArrowLeft } from "lucide-react"
...
export default function ReviewDetail({ bookID, reviewID, review }: ReviewDetailProps) {
  ...
  const handleMarkHelpful = async () => {
    ...
    setHelpfulCount((count: number) => (hasMarkedHelpful ? count - 1 : count + 1))
  ...
  const handleShareReview = () => {
    if (navigator.share) {
      navigator
        .share({
          title: `Review of ${title} by ${`${review.reviewer?.firstName} ${review.reviewer?.lastName}` || "Anonymous"}`,
  ...
  return (
    <div className="space-y-8">
        ...
        {!isAuthor && userID && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={handleReportReview} className="cursor-pointer" disabled={isReporting}>
                {isReporting ? (
                  <>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem asChild>
                <Link href={`/books/${bookID}/reviews/${review._id}/edit`} className="cursor-pointer">
                  <Edit className="mr-2 h-4 w-4" />
                  Edit review
                </Link>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => setShowDeleteDialog(true)} className="text-red-500 cursor-pointer">
      ...
      <Card className="overflow-hidden bg-transparent border-y-0 rounded-none">
        <CardContent className="px-6">
          <div className="flex items-start gap-4">
            <Avatar className="h-10 w-10 border">
              <AvatarFallback className={getRandomColor(review.reviewer?.firstName)}>
            ...
            <div className="flex-1">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div>
                  <h2 className="font-semibold font-inter text-[15px]">{review.reviewer?.fullName || "Anonymous"}</h2>
                  <time className="font-medium text-gray-400 text-xs" dateTime={review.createdAt}>
                    {formatDate(review.createdAt)}
                  </time>
                  {review.updatedAt !== review.createdAt && (
                    <span className="text-xs ml-1">(Updated on {formatDate(review.updatedAt)})</span>
                  )}
                </div>
              </div>

              <div className="mt-4 prose prose-gray dark:prose-invert max-w-none">
                <p className="text-sm leading-relaxed whitespace-pre-line break-words">{review.content}</p>
                {review.rating && (
        ...
        <CardFooter className="px-4 py-3 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-4">
            {!isAuthor && userID && (
              ...
              >
                {isMarkingHelpful ? (
                  ...
                  <ThumbsUp
                    className={`h-4 w-4 ${hasMarkedHelpful ? "fill-blue-500 text-blue-500" : "text-gray-300"}`}
                  />
                )}
                <span className={hasMarkedHelpful ? "text-blue-500" : "text-gray-300"}>Helpful ({helpfulCount})</span>
              </Button>
            )}
            ...
            {isAuthor && (
              <div className="text-sm text-gray-500 flex items-center gap-2">
                ...
                <span>
                  {helpfulCount} {helpfulCount === 1 ? "person" : "people"} helpful
                </span>
              </div>
            )}
          </div>
